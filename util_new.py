...
def load_data_n_model(dataset_name, model_name, root, sample_rate=1.0, sample_method='uniform_nearest',
    ...
    if dataset_name == 'UT_HAR_data':
        ...
        train_loader = torch.utils.data.DataLoader(
            dataset=CSI_Dataset(root + 'NTU-Fi-HumanID/train_amp/', sample_rate=sample_rate,
                                sample_method=sample_method, interpolation_method=interpolation_method,
                                use_energy_input=use_energy_input, use_mask_0=use_mask_0), batch_size=64, shuffle=True)
        test_loader = torch.utils.data.DataLoader(
            dataset=CSI_Dataset(root + 'NTU-Fi-HumanID/test_amp/', sample_rate=sample_rate, sample_method=sample_method,
                                interpolation_method=interpolation_method, use_energy_input=use_energy_input,
                                use_mask_0=use_mask_0), batch_size=64, shuffle=False)
        if model_name == 'MLP':
            print("using model: MLP")
        ...
        train_loader = torch.utils.data.DataLoader(
            dataset=CSI_Dataset(root + 'NTU-Fi_HAR/train_amp/', sample_rate=sample_rate, sample_method=sample_method,
                                interpolation_method=interpolation_method, use_energy_input=use_energy_input,
                                use_mask_0=use_mask_0),  # 别忘了使用你修正后的数据路径！
            batch_size=64,
            shuffle=True,
        ...
        test_loader = torch.utils.data.DataLoader(
            dataset=CSI_Dataset(root + 'NTU-Fi_HAR/test_amp/', sample_rate=sample_rate, sample_method=sample_method,
                                interpolation_method=interpolation_method, use_energy_input=use_energy_input,
                                use_mask_0=use_mask_0),  # 别忘了使用你修正后的数据路径！
            batch_size=64,
            shuffle=False,
