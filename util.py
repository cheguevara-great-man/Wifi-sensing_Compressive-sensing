...
def load_data_n_model(dataset_name, model_name, root):
    ...
    if dataset_name == 'UT_HAR_data':
        ...
        if model_name == 'MLP':
            ...
            print("using model: shuffleNet")
            model = UT_HAR_shufflenet_v2_x1_0(num_classes=7)
            train_epoch = 200 #100
        elif model_name == 'MobileNetV2':
            print("using model: MobileNetV2")
            model = UT_HAR_MobileNetV2(num_classes=7)
            train_epoch = 30 #100
        elif model_name == 'AlexNet':
            print("using model: AlexNet")
            model = UT_HAR_AlexNet(num_classes=7)
            train_epoch = 150 #100
        elif model_name == 'GoogleNet':
            print("using model: GoogleNet")
            model = UT_HAR_GoogLeNet(num_classes=7, aux_logits=True, init_weights=True)
            train_epoch = 150 #100
        elif model_name == 'VGG':
            print("using model: VGG")
            model = UT_HAR_VGG(model_name='vgg16', num_classes=7, init_weights=True)
            train_epoch = 150 #100
        elif model_name == 'LM':
            print("using model: LM")
            model = UT_HAR_LM()
            train_epoch = 150 #100
        elif model_name == 'LM_SGE':
            print("using model: LM_sge")
            model = UT_HAR_LM_SGE()
            train_epoch = 150 #100
        elif model_name == 'SGE':
            print("using model: SGE")
            model = UT_HAR_SGE()
            train_epoch = 150 #100
        elif model_name =='RecCls':
            print("using model: Rec+cls")
            model = UT_HAR_Recon_Classifier()
            train_epoch = 200
        elif model_name =='RecClsSGE':
            print("using model: Rec+cls+SGE")
            model = UT_HAR_Recon_Classifier_SGE()
            train_epoch = 200
