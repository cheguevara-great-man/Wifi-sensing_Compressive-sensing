...
folder_name = "UT_HAR_Rec_Cls_2block_SGE"
...
def train(model, tensor_loader, num_epochs, learning_rate, criterion, device):
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(model.state_dict(), os.path.join(log_path, folder_name, '{}_Rec_Cls_2block_SGE_MODEL.pth'.format(epoch + 1)))
...
def main():
    ...
    parser.add_argument('--model', choices = ['MLP','LeNet','ResNet18','ResNet50','ResNet101','RNN','GRU','LSTM','BiLSTM', 'CNN+GRU','ViT','LM','RecCls','RecClsSGE'], default='RecClsSGE')
    ...
    test(
        ...
        criterion=criterion,
        device= device,
        checkpoint_path='./checkpoints/UT_HAR_Rec_Cls_2block_SGE/67_Rec_Cls_2block_SGE_MODEL.pth',
        save_metrics_path='./Val/UT_HAR_Rec_Cls_2block_SGE/test_metrics.txt'
        )
    return
