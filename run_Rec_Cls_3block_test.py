...
def train(model, tensor_loader, num_epochs, learning_rate, criterion, device):
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(model.state_dict(),
                   os.path.join(log_path, folder_name, '{}_Rec_Cls_3block_MODEL.pth'.format(epoch + 1)))
...
def test(model, tensor_loader, criterion, device, checkpoint_path='None', save_metrics_path='None', save_images_path='None', tsne_perplexity=30):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_images_path, 'UT_HAR_Rec_Cls_3block_confusion.png'))
    #plt.show()
    ...
    plt.grid(True)
    plt.savefig(os.path.join(save_images_path, 'UT_HAR_Rec_Cls_3block_AUC_ROC.png'))
    #plt.show()
    print('********************ROC和AUC画图完毕******************************')
    ...
    plt.legend()
    #plt.grid(True)
    plt.savefig(os.path.join(save_images_path, 'UT_HAR_Rec_Cls_3block_Tsne.png'))
    #plt.show()
    print('********************Tsne画图完毕******************************')
...
def main():
    ...
    test(
        ...
        criterion=criterion,
        device=device,
        checkpoint_path='./checkpoints/UT_HAR_Rec_Cls_3block/200_Rec_Cls_3block_MODEL.pth',
        save_metrics_path='./Val/UT_HAR_Rec_Cls_3block/test_metrics.txt',
        save_images_path = './Val/UT_HAR_Rec_Cls_3block/'
    )
    return
