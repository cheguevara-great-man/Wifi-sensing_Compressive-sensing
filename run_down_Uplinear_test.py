...
folder_name = "UT_HAR_down_Uplinear_ResNet50"
...
def train(model, tensor_loader, num_epochs, learning_rate, criterion, device):
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(model.state_dict(),
                   os.path.join(log_path, folder_name, '{}_down_Uplinear_ResNet50_MODEL.pth'.format(epoch + 1)))
...
def test(model, tensor_loader, criterion, device, checkpoint_path='None', save_metrics_path='None', save_images_path='None'):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_images_path, 'UT_HAR_down_Uplinear_ResNet50_confusion.png'))
    plt.show()
    ...
    plt.grid(True)
    plt.savefig(os.path.join(save_images_path, 'UT_HAR_down_Uplinear_ResNet50_AUC_ROC.png'))
    plt.show()
    print('********************ROC和AUC画图完毕******************************')
...
def main():
    ...
    test(
        ...
        criterion=criterion,
        device=device,
        checkpoint_path='./checkpoints/UT_HAR_down_Uplinear_ResNet50/67_down_Uplinear_ResNet50_MODEL.pth',
        save_metrics_path='./Val/UT_HAR_down_Uplinear_ResNet50/test_metrics.txt',
        save_images_path = './Val/UT_HAR_down_Uplinear_ResNet50/'
    )
    return
